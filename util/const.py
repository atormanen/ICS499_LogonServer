...
class ConstContainerClass:
    ...
    def values(cls) -> Tuple[str]:
        ...
        return tuple([v for (k, v) in vars(cls).items() if cls._is_good_item(k, v)])

    # noinspection PyTypeChecker
    @classmethod
    def items(cls) -> Dict[str, str]:
        """Gets a list of all key-value pairs."""
        return [(k, v) for (k, v) in vars(cls).items() if cls._is_good_item(k, v)]
    ...
    def keys(cls) -> Tuple[str]:
        ...
        return tuple([k for (k, v) in vars(cls).items() if cls._is_good_item(k, v)])
